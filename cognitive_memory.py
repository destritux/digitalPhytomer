...
class HierarchicalMemory:
    ...
    def add_episode(self, prompt: str, response: str, evaluation_meta: dict, client=None):
        ...
        if len(self.episodic_log) >= self.capacity:
            self.abstract_to_semantic(client)

    def abstract_to_semantic(self, client):
        ...
        print("\n[Memory System] Capacity reached. Abstracting episodic logs into semantic memory...")
        ...
        try:
            ...
            # Save the semantic lesson into the shared somatic database for future searches
    ...
    def retrieve_context(self, query_text: str, client) -> str:
        ...
        try:
            ...
            context_parts = []
            if matches:
                context_parts.append("\n=== Recalled Semantic Knowledge (Retrieved Memory) ===")
                for m in matches:
    ...
    def get_memory_context(self) -> str:
        if not self.episodic_log:
            return ""
        parts = ["=== Episodic Local Memory (Recent Attempts on this task) ==="]
        ...
        parts.append("Learn from these previous attempts and correct any logical errors.")
        return "\n".join(parts)
